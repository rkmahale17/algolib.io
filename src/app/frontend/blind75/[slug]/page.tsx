...
import ProblemDetailClient from '@/app/problem/[slug]/ProblemDetailClient';
...
export async function generateMetadata({ params }: ProblemPageProps): Promise<Metadata> {
  ...
  if (!algorithm) {
    return {
      title: 'Problem Not Found | Frontend - RulCode',
    };
  }

  const baseTitle = `${algorithm.name} Visualizer & Solution | Frontend - RulCode`;
  const cleanTitle = baseTitle.length > 60 ? `${algorithm.name} | Frontend - RulCode` : baseTitle;
  ...
  return {
    ...
    alternates: {
      canonical: `/frontend/problem/${algorithm.id}`,
    },
    openGraph: {
      title: cleanTitle,
      description: cleanDesc,
      images: [algorithm.image || 'https://rulcode.com/og-image.png'],
      url: `/frontend/problem/${algorithm.id}`,
    },
  };
}
...
export default async function ProblemPage({ params }: ProblemPageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": algorithm.name,
        "item": `https://rulcode.com/frontend/problem/${algorithm.id}`
      }
    ]
  };
  ...
  const techArticleJsonLd = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://rulcode.com/frontend/problem/${algorithm.id}`
    },
    "about": {
      "@type": "Thing",
      "name": algorithm.name,
