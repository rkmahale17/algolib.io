...
import PatternGuessClient from './PatternGuessClient';
...
async function getAlgorithm(id: string) {
  ...
  const { data, error } = await client
    .from('algorithms')
    .select('*')
    .eq('id', id) // Existing logic uses ID as slug
    .maybeSingle();
...
export async function generateMetadata({ params }: ProblemPageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/pattern-guess/${algorithm.id}`,
    },
    openGraph: {
      title: cleanTitle,
      description: cleanDesc,
      images: [algorithm.image || 'https://rulcode.com/og-image.png'],
      url: `/pattern-guess/${algorithm.id}`,
    },
  };
}
