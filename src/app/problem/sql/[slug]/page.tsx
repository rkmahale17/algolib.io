...
import ProblemDetailClient from '../../[slug]/ProblemDetailClient';
...
export async function generateMetadata({ params }: ProblemPageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `/problem/sql/${algorithm.id}`,
    },
    openGraph: {
      title: cleanTitle,
      description: cleanDesc,
      images: [algorithm.image || 'https://rulcode.com/og-image.png'],
      url: `/problem/sql/${algorithm.id}`,
    },
  };
}
...
export default async function ProblemPage({ params }: ProblemPageProps) {
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 3,
        "name": algorithm.name,
        "item": `https://rulcode.com/problem/sql/${algorithm.id}`
      }
    ]
  };
  ...
  const techArticleJsonLd = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://rulcode.com/problem/sql/${algorithm.id}`
    },
    "about": {
      "@type": "Thing",
      "name": algorithm.name,
