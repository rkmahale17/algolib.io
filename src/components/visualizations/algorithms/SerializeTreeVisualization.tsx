...
function deserialize(data) {
  ...
  const generateSteps = () => {
    ...
    const serialize = (node: TreeNode | null) => {
      if (!node) {
        serialized.push('null');
        newSteps.push({
          current: null,
          serialized: [...serialized],
          message: 'Null node, add "null" to serialization',
          lineNumber: 1
        });
        return;
      }
      ...
      newSteps.push({
        current: node.val,
        serialized: [...serialized],
        message: `Serialize node ${node.val}`,
        lineNumber: 3
      });
    ...
    newSteps.push({
      current: null,
      serialized: [],
      message: 'Start serialization using preorder traversal',
      lineNumber: 0
    });
    ...
    newSteps.push({
      current: null,
      serialized: [...serialized],
      message: `Complete! Serialized: ${serialized.join(',')}`,
      lineNumber: 6
    });
      ...
      <g key={`${node.val}-${node.x}`}>
        ...
        <circle
          ...
          className={`transition-all duration-300 ${
            currentStep.current === node.val ? 'fill-primary stroke-primary' : 'fill-muted stroke-border'
          }`}
          strokeWidth="2"
        />
        <text
          ...
          textAnchor="middle"
          dy=".3em"
          className={`font-bold ${currentStep.current === node.val ? 'fill-white' : 'fill-foreground'}`}
        >
          {node.val}
        </text>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="space-y-4">
          ...
          <div className="bg-blue-500/10 border border-blue-500/30 rounded-lg p-4">
            <p className="text-xs text-muted-foreground mb-2">Serialized String:</p>
