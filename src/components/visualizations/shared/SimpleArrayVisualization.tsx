        ...
        {array.map((value, index) => {
          ...
          return (
            <div key={index} className="flex flex-col items-center gap-2 flex-1 max-w-[80px] flex-wrap">
              <div
                className={`w-full rounded-t transition-all duration-300 flex items-end justify-center pb-2 ${isHighlighted ? 'bg-primary' : 'bg-muted'
                  }`}
                style={{ height: `${height}%`, minHeight: '40px' }}
              >
                <span className={`text-sm font- ${isHighlighted ? 'text-primary-foreground' : 'text-foreground'}`}>
                  {value}
                </span>
              </div>
